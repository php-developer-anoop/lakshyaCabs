                        ...
                        if(!empty($triptype)){
                            ...
                            <a href="<?=base_url(ADMINPATH)?>add-<?=strtolower($value['trip_type'])?>-fare">
                                <li class="nav-item">
                                    <button 
                                    type="button"
                                    class="nav-link <?=empty($value) || ($value['trip_type'] =="Oneway")?"active":""?> "
                                    role="tab"
                                    data-bs-toggle="tab"
                                    data-bs-target="#navs-pills-top-oneway"
                                    aria-controls="navs-pills-top-oneway"
                                    aria-selected="<?=empty($value) || ($value['trip_type'] =="Oneway")? true : false ?>">
                  ...
                  <div class="tab-pane fade <?=empty($type) || ($type=="Oneway")?"show active":""?>" id="navs-pills-top-oneway" role="tabpanel">
                    ...
                    <div class="row mb-3">
                      <div class="col-sm-3">
                          ...
                          <select name="pickup_state_id" id="pickup_state_id" class="form-control select2" onchange="changestate('pickup_state_id','pickup_city_id')" required>
                            <option value="">--Select State--</option>
                            <?php 
                            if(!empty($statelist)){
                              foreach ($statelist as $key => $value) { ?>
                            <option <?php if( $value['id'] == $pickup_state_id ){echo 'selected';}?> value="<?=!empty($value['id']) ? $value['id'] : ""?>"><?=!empty($value['state_name']) ? $value['state_name'] : ""?></option>  
                            <?php  } }?>
                          </select>
                        </div>
                        ...
                        <div class="col-sm-3">
                          <?=form_label('Select Pickup City','pickup_city',['class'=>'col-form-label'])?> 
                            <select class="selectpicker" name="pickup_city_id[]" id="pickup_city_id" multiple aria-label="Default select example" data-placeholder="Choose Option" data-live-search="true">
                              
                          </select>
                        </div>
                      <div class="col-sm-3">
                        <?=form_label('Select Drop City','drop_city',['class'=>'col-form-label'])?>
                        <input type="text" name="drop_city" autocomplete="off" onkeyup="getDropCityList(this.value)" value="<?=$drop_city_name?>"  class="form-control ucwords restrictedInput" id="drop_city_name" placeholder="Drop City Name" />
                        <input type="hidden" name="drop_city_id" id="drop_city_id" value="<?=$drop_city_id?>">
                        <input type="hidden" name="drop_state_id" id="drop_state_id" value="<?=$drop_state_id?>">
                        <ul class="drop-autocomplete-list list-unstyled list-color" id="drop-suggestion-list1" onclick="return selectDropCityName()"></ul>
                      </div>
                    </div>
                    <div class="row">
                    <div class="col-sm-10">
                    <div class="row mb-2 plusminus">
                       ...
                       <div class="custom_fields">
                        <label class="col-form-label">Select Vehicle Model</label>
                            ...
                            <input type="text" name="key[0][base_covered_km]" placeholder="Enter value" class="form-control" value="<?=$base_covered_km?>">
                            </div>
                            <div class="custom_fields">
                      ...
                      <div class="col-sm-6">
                         <div class="dlfexend">
                            <?php if(!empty($access) || ($user_type != "Role User") ){?>
                                  <button type="submit" id="submit" class="btn btn-primary"><?=$btn?></button>
                            <?php } ?>
                         </div>
                      </div>
                    </div>
  ...
  addBlockBtn.addEventListener('click', function() {
    ...
    const newField = `
      ...
      <div class="custom_fields">
        ...
        <div class="custom_fields">
          <label class="col-form-label">Base Fare</label>
          <input type="text" name="key[${fieldIndex}][base_fare]" placeholder="Enter fare" class="form-control">
        </div>
        <div class="custom_fields">
          <label class="col-form-label">Rate per KM</label>
          <input type="text" name="key[${fieldIndex}][per_km_charge]" placeholder="Enter rate" class="form-control">
        </div>
        <div class="custom_fields">
          <label class="col-form-label">Minimum KM</label>
          <input type="text" name="key[${fieldIndex}][base_covered_km]" placeholder="Enter value" class="form-control">
        </div>
        <div class="custom_fields">
          <label class="col-form-label">Driver Charge(DA)</label>
          <input type="text" name="key[${fieldIndex}][driver_charge]" placeholder="Enter value" class="form-control">
        </div>
        <div class="custom_fields">
          <label class="col-form-label">Night Charge</label>
          <input type="text" name="key[${fieldIndex}][night_charge]" placeholder="Enter value" class="form-control">
        </div>
      </div>
    `;
