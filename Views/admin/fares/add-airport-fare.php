              ...
              <div class="nav-align-top mb-4">
                        ...
                        if(!empty($triptype)){
                            ...
                            <a href="<?=base_url(ADMINPATH)?>add-<?=strtolower($value['trip_type'])?>-fare">
                                <li class="nav-item">
                                    <button 
                                    type="button"
                                    class="nav-link <?=empty($value) || ($value['trip_type'] =="Airport")?"active":""?> "
                                    role="tab"
                                    data-bs-toggle="tab"
                                    data-bs-target="#navs-pills-top-oneway"
                                    aria-controls="navs-pills-top-oneway"
                                    aria-selected="<?=empty($value) || ($value['trip_type'] =="Airport")? true : false ?>">
                ...
                <div class="tab-content">
                  <div class="tab-pane fade <?=empty($type) || ($type=="Oneway")?"show active":""?>" id="navs-pills-top-oneway" role="tabpanel">
                    <?=form_open(ADMINPATH . 'save-fare-airpoart',['id'=>'Oneway_form']); ?>
                    <?=form_hidden('id',$id)?>
                    <?=form_hidden('trip_type','Airport')?>
                    <div class="row mb-3">
                      <div class="col-sm-3">
                        <?=form_label('Select Pickup City','drop_city',['class'=>'col-form-label'])?>
                        <input type="text" name="pickup_city_name" autocomplete="off" onkeyup="getDropCityList(this.value)" value="<?=$pickup_city_name?>"  class="form-control ucwords restrictedInput " required id="pickup_city_name" placeholder="Pickup City Name" />
                        <input type="hidden" name="pickup_city_id" id="pickup_city_id" onchange="getPackage('')" value="<?=$pickup_city_id?>">
                        <input type="hidden" name="pickup_state_id" id="pickup_state_id" value="<?=$pickup_state_id?>">
                        <ul class="drop-autocomplete-list list-unstyled list-color" id="drop-suggestion-list1" onclick="return selectDropCityNames()"></ul>
                      </div>
                     
                      <?php ?>
                      <div class="col-sm-3">
                        <?=form_label('Select Airport','pickup_city',['class'=>'col-form-label'])?>  
                        <select name="package_id" id="package_id" class="form-control select2"  required>
                          <option value="">--Select Airport--</option>
                          
                           <?php 
                                if(!empty($airportl)){
                                foreach ($airportl as $key => $value) { ?>
                                    <option <?php if( $value['id'] == $package_id ){echo 'selected';}?> value="<?=!empty($value['id']) ? $value['id'] : ""?>"><?=!empty($value['airport_name']) ? $value['airport_name'] : ""?></option>  
                                <?php  } }?>
                        </select>
                      </div>
                       <?php ?>
