    ...
    <div class="card pt-2">
      <div class="card-body">
          <div class="dflexbtwn hdngvwall">
                <h4 class="cardhdng">List State</h4>
                <a href="<?= base_url(ADMINPATH . 'add-state') ?>" class="sitebtn">Add</a>
          </div>
...
<script>
    ...
    function loadAllRecordsData(qparam) {
       // alert(qparam);
        ...
        $('#responseData').DataTable({
            ...
            { data: "", "title": "State Name","render":state_detail },
    ...
    var state_detail = (data, type, row, meta) => {
  var data = '';
  let state_name = row.state_name != null ? row.state_name : "";
  let state_code = row.state_code != null ? row.state_code : "";
  if (type === 'display') {
    data += '<span class="fotr_10"><b>Name : </b>' + state_name + '</span><br>';
    data += '<span class="fotr_10"><b>Code: </b>' + state_code + '</span>';
...
function action_render(data, type, row, meta) {
  let output = '';
  if (type === 'display') {
    var onclick = "remove('" + row.id + "','dt_states')";
    output = '<a href="<?= base_url(ADMINPATH . "add-state?id=") ?>' + row.id + '" class="btn actnbtn btn-orange" title="Edit State"><i class="tf-icons bx bx-edit"></i></a> ';
