    ...
    <div class="card pt-2">
      <div class="card-body">
          <div class="dflexbtwn hdngvwall">
                ...
                <a href="<?= base_url(ADMINPATH . 'add-cms') ?>" class="sitebtn">Add</a>
          </div>
...
<script>
    ...
    function loadAllRecordsData(qparam) {
       // alert(qparam);
        ...
        $('#responseData').DataTable({
            ...
            { data: "", "title": "Page Detail","render":page_detail},
  ...
  let url= row.page_slug!=null?row.page_slug:"";
  if(type === 'display'){
        data += '<span class="fotr_10"><b>Page Name : </b>'+page_name+'</span><br>' ;
        data += '<span class="fotr_10"><b>Page Slug : </b><a href='+page_url+' target="_blank">'+url+'</a></span>' ;
...
function action_render(data, type, row, meta) {
  let output = '';
  if (type === 'display') {
    var onclick = "remove('" + row.id + "','dt_cms')";
    output = '<a href="<?= base_url(ADMINPATH . "add-cms?id=") ?>' + row.id + '" class="btn actnbtn btn-orange" title="Edit CMS"><i class="tf-icons bx bx-edit"></i></a> ';
    output += '<a class="btn actnbtn btn-red" onclick="' + onclick + '" title="Delete CMS"><i class="tf-icons bx bx-trash"></i></a> ';
  }
  return output;
}
