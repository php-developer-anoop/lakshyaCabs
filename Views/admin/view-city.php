    ...
    <div class="card pt-2">
        <div class="card-body">
          <div class="dflexbtwn hdngvwall">
                <h4 class="cardhdng">City List</h4>
                <a href="<?= base_url(ADMINPATH . 'add-city') ?>" class="sitebtn">Add</a>
          </div>
    ...
    function loadAllRecordsData(qparam) {
       // alert(qparam);
        ...
        $('#responseData').DataTable({
            ...
            { data: "", "title": "City Details","render":city_details },
...
var city_details = (data, type, row, meta) => {
  var data = '';
...
function action_render(data, type, row, meta) {
  let output = '';
  if (type === 'display') {
    var onclick = "remove('" + row.id + "','dt_cities')";
    output = '<a href="<?= base_url(ADMINPATH . "add-city?id=") ?>' + row.id + '" class="btn actnbtn btn-orange" title="Edit City"><i class="tf-icons bx bx-edit"></i></a> ';
   // output += '<a class="btn btn-sm btn-danger text-white" onclick="' + onclick + '" title="Delete City"><i class="tf-icons bx bx-trash"></i></a> ';
  }
  return output;
}
...
function status_render(data, type, row, meta) {
  if (type === 'display') {
    ...
    const onchange = `change_status(${row.id}, 'dt_cities')`;
