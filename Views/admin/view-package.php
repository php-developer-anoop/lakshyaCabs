    ...
    <div class="card pt-2">
        <div class="card-body">
          <div class="dflexbtwn hdngvwall">
              <h4 class="cardhdng">Package List</h4>
              <a href="<?= base_url(ADMINPATH . 'add-package') ?>" class="sitebtn">Add</a>
          </div>
...
<script>
    function getTotalRecordsData(qparam) {
        $.ajax({
            url: '<?= base_url(ADMINPATH . 'package-data'); ?>?' + qparam,
            type: "POST",
    ...
    function loadAllRecordsData(qparam) {
       // alert(qparam);
        ...
        $('#responseData').DataTable({
            ...
            "serverSide": true,
            "ajax": {
                "url": '<?= base_url(ADMINPATH . 'package-data'); ?>' + newQueryParam,
                "type": 'POST',
                dataSrc: (res) => {
                    return res.data
                }
...
function action_render(data, type, row, meta) {
  let output = '';
  if (type === 'display') {
    var onclick = "remove('" + row.id + "','dt_package_list')";
    output = '<a href="<?= base_url(ADMINPATH . "add-package?id=") ?>' + row.id + '" class="btn actnbtn btn-orange" title="Edit Package"><i class="tf-icons bx bx-edit"></i></a> ';
