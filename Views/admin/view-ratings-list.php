...
<script>
    function getTotalRecordsData(qparam) {
        $.ajax({
            url: '<?= base_url(ADMINPATH . 'ratings-data'); ?>?' + qparam,
            type: "POST",
    ...
    function loadAllRecordsData(qparam) {
       // alert(qparam);
        ...
        $('#responseData').DataTable({
            ...
            "serverSide": true,
            "ajax": {
                "url": '<?= base_url(ADMINPATH . 'ratings-data'); ?>' + newQueryParam,
                "type": 'POST',
                dataSrc: (res) => {
                    return res.data
                }
            ...
            { data: "id", "title": "Ratings Details","render":rating_detail },
            { data: "id", "title": "Description","render":description },
    ...
    var rating_detail = (data, type, row, meta) => {
  var data = '';
  let booking_id = row.booking_id != null ? row.booking_id : "";
  let customer_name = row.customer_name != null ? row.customer_name : "";
  let rating = row.rating != null ? row.rating : "";
  if (type === 'display') {
    data += '<span class="fotr_10"><b>Booking ID : </b>' + booking_id + '</span><br>';
    data += '<span class="fotr_10"><b>Customer Name: </b>' + customer_name + '</span><br>';
    data += '<span class="fotr_10"><b>Rating: </b>' + rating + '</span>';
  }
  return data;
}
  var description = (data, type, row, meta) => {
  var data = '';
  let description = row.description != null ? row.description : "";
  
  if (type === 'display') {
    data += '<span class="fotr_10">' + description + '</span>';
  }
  return data;
}
...
function action_render(data, type, row, meta) {
  let output = '';
  if (type === 'display') {
    var onclick = "remove('" + row.id + "','dt_ratings_list')";
    output += '<a class="btn btn-sm btn-danger text-white" onclick="' + onclick + '" title="Delete Rating"><i class="tf-icons bx bx-trash"></i></a> ';
  }
  return output;
}
...
function status_render(data, type, row, meta) {
  if (type === 'display') {
    ...
    const onchange = `change_status(${row.id}, 'dt_ratings_list')`;
