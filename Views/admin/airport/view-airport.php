    ...
    <div class="card pt-2">
     <div class="card-body">
      <div class="dflexbtwn hdngvwall">
              <h4 class="cardhdng">Airport list </h4>
              <a href="<?= base_url(ADMINPATH . 'add-airport') ?>" class="sitebtn">Add Airport</a>
      </div>
...
<script>
    function getTotalRecordsData(qparam) {
        $.ajax({
            url: '<?= base_url(ADMINPATH . 'airport-data'); ?>?' + qparam,
            type: "POST",
    ...
    function loadAllRecordsData(qparam) {
       // alert(qparam);
        ...
        $('#responseData').DataTable({
            ...
            "serverSide": true,
            "ajax": {
                "url": '<?= base_url(ADMINPATH . 'airport-data'); ?>' + newQueryParam,
                "type": 'POST',
                dataSrc: (res) => {
                    return res.data
                }
            ...
            { data: "", "title": "Airport Details","render":package_details },
...
var package_details = (data, type, row, meta) => {
  var data = '';
  let airport_name = row.airport_name != null ? row.airport_name : "";
  let city_name = row.city_name != null ? row.city_name : "";
  if (type === 'display') {
    data += '<span class="fotr_10"><b>Airport Name : </b>' + airport_name + '</span><br>';
...
function action_render(data, type, row, meta) {
  let output = '';
  if (type === 'display') {
    var onclick = "remove('" + row.id + "','dt_airport')";
    output = '<a href="<?= base_url(ADMINPATH . "add-airport?id=") ?>' + row.id + '" class="btn actnbtn btn-orange" title="Edit Hourly Package"><i class="tf-icons bx bx-edit"></i></a> ';
...
function status_render(data, type, row, meta) {
  if (type === 'display') {
    ...
    const onchange = `change_status(${row.id}, 'dt_airport')`;
