    ...
    <div class="card pt-2">
      <div class="card-body">
          <div class="dflexbtwn hdngvwall">
                 <h4 class="cardhdng">Why Choose Us </h4>
                 <a href="<?= base_url(ADMINPATH . 'add-choose') ?>" class="sitebtn">Add Reason</a>
          </div>
...
<script>
    function getTotalRecordsData(qparam) {
        $.ajax({
            url: '<?= base_url(ADMINPATH . 'choose-data'); ?>?' + qparam,
            type: "POST",
    ...
    function loadAllRecordsData(qparam) {
       // alert(qparam);
        ...
        $('#responseData').DataTable({
            ...
            "serverSide": true,
            "ajax": {
                "url": '<?= base_url(ADMINPATH . 'choose-data'); ?>' + newQueryParam,
                "type": 'POST',
                dataSrc: (res) => {
                    return res.data
                }
...
function action_render(data, type, row, meta) {
  let output = '';
  if (type === 'display') {
    var onclick = "remove('" + row.id + "','dt_why_choose_us')";
    output = '<a href="<?= base_url(ADMINPATH . "add-choose?id=") ?>' + row.id + '" class="btn actnbtn btn-orange" title="Edit Reason"><i class="tf-icons bx bx-edit"></i></a> ';
...
function status_render(data, type, row, meta) {
  if (type === 'display') {
    ...
    const onchange = `change_status(${row.id}, 'dt_why_choose_us')`;
