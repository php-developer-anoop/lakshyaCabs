...
class Recharge_plan extends BaseController {
    ...
    function add_plan() {
        ...
        $data['access']=checkWriteMenus(getUri(2));
        $data["menu"] = "Recharge Plan Master";
        $data["title"] = !empty($id) ? "Edit Plan" : "Add Plan";
        $savedData = $this->c_model->getSingle($this->table, '*', ['id' => $id]);
        $data['id'] = !empty($savedData['id']) ? $savedData['id'] : $id;
        $data['amount'] = !empty($savedData['amount']) ? $savedData['amount'] : '';
        $data['cashback_amount'] = !empty($savedData['cashback_amount']) ? $savedData['cashback_amount'] : '';
