...
class CmsMaster extends BaseController {
    ...
    public function index() {
        ...
        $getInput = $this->request->getVar();
        $id = !empty($getInput['id']) ? $getInput['id'] : ''; 
        ...
        $data["menu"] = "City/Tabs CMS Master";
        $data["title"] = !empty($id) ? "Edit City/Tabs CMS" : "Add City/Tabs CMS";
        
        
        $savedData = !empty($id) ? $this->c_model->getSingle($this->table, '*', ['id' => $id]) : [];
        ...
        $data['trip_type_list'] =  dropDownList('dt_cms_tab_master', ['status'=>'Active'], 'tab_name', 'trip_type','Select Trip Type');
    ...
    public function save_route() {
        ...
        $saveData['page_schema'] = trim($post['page_schema']);
    ...
    public function getRecords() {
        ...
        $url = base_url();
        $select = 'id,page_name,status,page_slug,CONCAT("'.base_url('').'",page_slug) as page_url,DATE_FORMAT(add_date , "%d-%m-%Y %r") AS add_date,DATE_FORMAT(updated_on , "%d-%m-%Y %r") AS update_date,banner_image_jpg,banner_image_webp,footer_image_jpg,footer_image_webp';
