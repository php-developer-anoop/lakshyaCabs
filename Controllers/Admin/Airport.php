...
class Airport extends BaseController {
    ...
    function add_airport() {
        ...
        $data['access']=checkWriteMenus(getUri(2));
        $data["menu"] = "Airport Master";
        $data["title"] = !empty($id) ? "Edit Airport" : "Add Airport";
        $savedData = $this->c_model->getSingle($this->table, '*', ['id' => $id]);
        $data['id'] = !empty($savedData['id']) ? $savedData['id'] : $id;
        $data['airport_name'] = !empty($savedData['airport_name']) ? $savedData['airport_name'] : '';
        ...
        $data['city_name']=!empty($state_name&&$city_name)?$city_name.' , '.$state_name:$city_name;
        $data['status'] = !empty($savedData['status']) ? $savedData['status'] : 'Active';
        adminview('airport/add-airport', $data);
    }
    public function save_airport() {
        $post = $this->request->getVar();
        // echo "<pre>";
        // print_r($post);exit;
        $id = !empty($this->request->getVar('id')) ? $this->request->getVar('id') : '';
        $data = [];
        $data['airport_name'] = ucwords(trim($post['airport_name']));
        ...
        if ($duplicate && empty($id)) {
            ...
            return redirect()->to(base_url(ADMINPATH . 'save-airport'));
        }
    ...
    public function getRecords() {
        ...
        if (!empty($get["search"]["value"])) {
            ...
            $where["airport_name LIKE '%" . $searchString . "%'"] = null;
            $limit = 100;
            $start = 0;
        }
