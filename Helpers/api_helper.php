...
if (!function_exists('generateBookingNumber')) { 
    function generateBookingNumber($lastBookingNumber) {  
        $prefix = strtoupper(substr($_SERVER['SERVER_NAME'], 0, 3));
        $uid = $prefix.date('Y').sprintf("%06d", $lastBookingNumber);
        return $uid; 
    }
}

if (!function_exists('generateDriverUIDNumber')) { 
    function generateDriverUIDNumber($lastId) {  
        $prefix = strtoupper(substr($_SERVER['SERVER_NAME'], 0, 3));
        $uid = $prefix.'D'.date('Y').sprintf("%06d", $lastId);
        return $uid; 
    }
}

if (!function_exists('generateCommaToArrayWithTwoItems')) { 
    function generateCommaToArrayWithTwoItems( $input_string ) {  
        $items_array = explode(",", $input_string);  
        $result = [];
        for ($i = 0; $i < count($items_array); $i += 2) { 
            if (isset($items_array[$i + 1])) { 
                $result[] = $items_array[$i].', '.$items_array[$i + 1]; 
            } else {
                $result[] = $items_array[$i];
            }
        }
        return $result;
    }
